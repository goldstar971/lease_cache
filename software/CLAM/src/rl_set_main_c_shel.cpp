...
#include <fstream>
#include <limits>
#include "reference_lease_set_c_shel.h"
using namespace std;
...
void process(string fileName, int num_way, int num_capacity, int* sample_distance) {
...
int main(int argc, char** argv) {
    ...
    if (argc != 4) {
        cout << "executable takes two arguments: sample file, cache size(KB) num_ways";
    	return 1;
	} else {
        fileName = argv[1];
        ...
        num_way=stoi(argv[3]);
  		...
  		printf("num_capacity: %d\n Num_way: %d\n",num_capacity, num_way);
    int sample_distance = 1000;
    
    // data block size is 8 Byte
    process(fileName, num_way, num_capacity, &sample_distance);
