...
def build_ri_distributions(trace_file):
    ...
    for line in lines:
        cells = line.split(',')
        #cells[0]: phase x address (string)
        #cells[1]: ri
                #cells[2]: tag
        #cells[3]: logical time
        a = interpret_address(cells[0])[1]
        if a not in distributions:
            #construct new ri_histogram object
            hist = ri_histogram(a)
            #add it to distributions
            distributions[a] = hist 
        else:
            hist = distributions[a]

        #increment value of the correct histogram
        if(interpret_hex(cells[1])<0):
            hist.add_ri(interpret_hex('FFFFEEEE'))
        else:
...
def get_max_ppuc_single(distribution,current_lease):
    ...
    for ri,freq in ris.items():
        ...
        if ri > current_lease:
            if MAX_LEASE_LIMITED and ri> MAX_LEASE:
                continue
...
def get_binned_hists(trace_file,bin_width):
    ...
    for line in lines:
        ...
        a = interpret_address(cells[0])[1]
        if a not in curr_bin_dict:
            curr_bin_dict[a]=1
        else:
            curr_bin_dict[a]+=1

        #handle ri distribution dict
        if a not in curr_ri_distribution_dict:
            #construct new ri_histogram object
            hist = ri_histogram(a)
            #add it to curr_ri_distribution_dict
            curr_ri_distribution_dict[a] = hist 
        else:
            hist = curr_ri_distribution_dict[a]
        #increment value of the correct histogram
        hist.add_ri(interpret_hex(cells[1]))
...
def phased_CARL(addrs,binned_ri_distributions,binned_freq_hists,cache_size,carl_order,bin_width,consensus,sample_rate):
    ...
    for lease_assignment in carl_order:
        if MAX_LEASE_LIMITED and lease_assignment.ri > MAX_LEASE:
            print("MAX LEASE EXCEEDED. PASSING _______________________________________")
            continue  
...
#       print(d)
    ...
    if(MAX_LEASE_LIMITED):
        print(f"Warning: Maximum lease = {MAX_LEASE}")
    print("DUMP LEASES")
    ...
    for address,lease in leases.items():
        lease_value=lease if lease else 1;
        if(address in duals):
            print(f"0, {hex(address)[2:]}, {hex(lease_value)[2:]}, {hex(duals[address][0])[2:]}, {1 - duals[address][1]} ")
        else:
            print(f"0, {hex(address)[2:]}, {hex(lease_value)[2:]}, 0, 1 ")
