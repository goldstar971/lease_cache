...
unset rate multi_level seed ways llt_size capacity
unset OPTIND
#get options
while getopts "m:r:s:w:l:c:" option; do
	case "$option" in
        l ) llt_size="$OPTARG";;
        r ) rate="$OPTARG";;
        s ) seed="$OPTARG";;
...
if [[ "$multi_level" == "yes" ]]; then
	capacity=${capacitiy-512}
	ways=${ways-$capacity}
else 
	capacity=${capacitiy-128}
	ways=${ways-$capacity}
fi

read -p "Generate leases for small dataset? [Y/n]?" do_small
read -p "Generate leases for medium dataset [Y/n]?" do_medium
read -p "Generate leases for large dataset? [Y/n]?" do_large
...
if [[ "$multi_level" == "yes" ]]; then
	...
	if [[ "$do_medium" == "y" ]] || [[ "$do_medium" == "Y" ]]; then
