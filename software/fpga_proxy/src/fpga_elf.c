...
uint32_t dma_elf_data(pHandle pInst, char *filename, uint32_t address, uint32_t size, uint32_t bytes_sent, uint32_t write_flag){
	...
	while(bytes_remaining){
		...
		// ---------------
		if (write_flag){
			if (protocol_write(pInst, pBuffer, file_size, memory_address,0)){
				return 1;
			}
			...
			fflush(stdout);		
				...
				// read data
				if (protocol_read(pInst, pBufferVerify, file_size, memory_address,0)){
					return 1;
				}
				// check data
				int verification_fail=0;
				for (uint32_t k = 0; k < file_size; k = k + 1){
					int num_tries=0;
					while(pBuffer[k] != pBufferVerify[k]){
						num_tries++;
						//attempt to rewrite correct word to memory
						protocol_write(pInst,pBuffer+(k/4)*4,4,memory_address+(k/4)*4,0);
						protocol_read(pInst,pBufferVerify+(k/4)*4,4,memory_address+(k/4)*4,0);
						if(num_tries==NUM_VERIFICATION_ATTEMPTS){
							printf("\nVerification failed\n");
							fflush(stdout);	
							return 1;
						}
