...
for size_index=2:2
	...
	pm_table=projected_misses_table_all(strcmp(projected_misses_table_all.Dataset_Size,dataset_size(size_index))&projected_misses_table_all.cache_size==128,:);
	%get benchmark names
	...
	%get plru and benchmark run data
		...
		'Location','northeastoutside','Orientation','Vertical');
			...
			%place total number of references underneath label
			if (scope_index==1)
				plru_total_references=plru_total_references_single_scope;
			else
				plru_total_references=plru_total_references_multi_scope;
			end
			for z=1:length(benchmark_labels)
				references=text(0,0,strcat('(',num2str(plru_total_references(z)),')'),'HorizontalAlignment','left'...
						,'VerticalAlignment','bottom','Units','normalized');
					references.Position=[1/length(benchmark_labels)*(z+.05),-.18];
			...
			saveas(fig,[base_save_path,'cache_statistics/cache_statistics_graphs/',convertStringsToChars(num_level(multi_level+1)),...
				'/sensitivity_',convertStringsToChars(plot_types(policy_index)),'_',convertStringsToChars(num_scope(scope_index)),'_',convertStringsToChars(dataset_size(size_index)),'.png'])
			close(fig);
		end
	end
